-- SAR Data Capture Script Revision 10
-- Automates capturing 40 scans for SAR processing with integrated Gantry control.
-- Based on sar_scan_rev9.lua.
-- Changes:
--   - ALL Gantry commands (X and Y) are now ASYNCHRONOUS (Non-blocking).
--   - This eliminates waiting for SSH overhead/teardown.
--   - We wait only for the Frame Duration + Safety Buffer (8.2s total), which covers the physical motor time (~7.8s).
...
function RunRemoteCommandAsync(args)
    ...
    -- This creates a "headless" feel as the window is minimized and doesn't block execution.
...
-- INITIALIZATION
-- =================================================================================
WriteToLog("Starting SAR Scan Revision 10 (Fully Async)...\n", "blue")
...
for y = 1, num_y_steps do
    ...
    -- 3. Trigger Gantry Motion (X-Axis) - ASYNC
    local move_args = ""
    if (y % 2 ~= 0) then
    ...
    -- Returns immediately. Motor starts moving ~250ms later.
    RunRemoteCommandAsync(move_args)
    
    -- 4. Wait for Frame Completion
    -- We wait for the Frame Duration + Buffer.
    -- Frame: 7.2s. Buffer: 1.0s. Total: 8.2s.
    -- Physical Motor Time: 280mm / 36mm/s = ~7.8s.
    -- 8.2s > 7.8s, so this is safe. We don't wait for SSH overhead.
