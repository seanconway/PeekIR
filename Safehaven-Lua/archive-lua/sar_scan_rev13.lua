...
-- Dynamic Directory Search to find next dumpsN folder
local p = io.popen('dir "' .. root_path .. '" /b /ad')
if p then
    for filename in p:lines() do
        -- Match folders starting with "dumps" followed by a number (e.g. dumps0, dumps1 - object3)
...
-- Frame Config
-- apparently changing second number from 1 to 0 makes it simiple TIMO instead of MIMO
...
for y = 1, num_y_steps do
    ...
    RSTD.Sleep(0) -- Wait for DCA to arm
    
    -- 2. Trigger Gantry Motion (X-Axis) - ASYNC
    -- RASTER SCAN: Always move RIGHT for the scan
    ...
    -- RASTER SCAN: Always trigger (since we are always moving Right)
    ...
    -- 5. Return X-Axis (Left) and Step Up
    ...
    -- Wait for return (Distance / Speed)
    -- 280mm / 36mm/s ~= 7777ms. Add buffer.
