# Revision 14  (based on rev 13)
# Changes from rev 13:
#   - Replaced `silent` with `--quiet` so MOTOR_STARTED/STOPPED always reach stdout
#   - Added epoch timestamps to MOTOR_STARTED / MOTOR_STOPPED signals
#   - Added `scan` command for autonomous snake-pattern SAR scanning with timing log
#
# Quick CLI usage:
#   python3 motorTest_rev14.py next
...
#   python3 motorTest_rev14.py scan --rows 40 --x-travel 280 --y-step 1 speed=18mms
...
QUIET_MODE = "--quiet" in sys.argv or "quiet" in sys.argv

def qprint(*args, **kwargs):
    """Print that respects --quiet mode. Timing signals use print() directly."""
    if not QUIET_MODE:
        print(*args, **kwargs)
...
def up(dist_mm):
    ...
    print(f"MOTOR_STARTED {time():.6f}", flush=True)
    if sleep_with_limit_check(duration, 'y', 1):
        print("Limit hit! Stopping.")
    pulY.stop()
    print(f"MOTOR_STOPPED {time():.6f}", flush=True)
...
def main_logic():
    # --quiet is handled at module level (QUIET_MODE flag).
    ...
    if "facetrack" in sys.argv:
        qprint("DEBUG: Executing 'facetrack' command (Live Average)")
    ...
    if "scan" in sys.argv:
        ...
        for idx, a in enumerate(args):
            if a == "--rows" and idx + 1 < len(args):
                scan_rows = int(args[idx + 1])
            elif a == "--x-travel" and idx + 1 < len(args):
                scan_x_travel = int(args[idx + 1])
            elif a == "--y-step" and idx + 1 < len(args):
                scan_y_step = int(args[idx + 1])

        T0 = monotonic()
        print(f"SCAN_STARTED {time():.6f}", flush=True)

        for row in range(1, scan_rows + 1):
            direction = "right" if row % 2 == 1 else "left"
            move_fn = right if direction == "right" else left

            elapsed = monotonic() - T0
            print(f"ROW {row} {direction} STARTED {elapsed:.6f}", flush=True)

            move_fn(scan_x_travel)

            elapsed = monotonic() - T0
            print(f"ROW {row} STOPPED {elapsed:.6f}", flush=True)

            if row < scan_rows:
                elapsed = monotonic() - T0
                print(f"STEP {row} STARTED {elapsed:.6f}", flush=True)
                up(scan_y_step)
                elapsed = monotonic() - T0
                print(f"STEP {row} STOPPED {elapsed:.6f}", flush=True)

        elapsed = monotonic() - T0
        print(f"SCAN_COMPLETE {elapsed:.6f}", flush=True)
        stopAllMotor()
        return

    # Priority order: next -> origin -> directional commands
    if "next" in sys.argv:
        qprint("DEBUG: Executing 'next' command")
        # Load current index
        if os.path.exists("current_index.txt"):
            with open("current_index.txt", "r") as f:
                current_index = int(f.read().strip())
    ...
    if origin_arg:
        qprint(f"DEBUG: Executing 'origin' command (Target: {origin_arg})")
    ...
    if "reset" in sys.argv:
        qprint("DEBUG: Executing 'reset' command")
